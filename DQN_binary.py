...
class SelectActionNet:
    ...
    def select_action(self, model):
        ...
        data_feature = np.stack((step_feature, features.expected, features.pro_lower_bound, features.pro_upper_bound,
                                 features.interval_length, features.num_selection/25, features.num_opp_selection/25,
                                 features.sum_num/50, features.num_ratio, features.ratio_reward, features.get_reward_th,
                                 features.num_opp_before/10, features.num_before/10, features.num_opp_after/10,
                                 features.num_after/10, features.opp_bunching, features.opp_rep_trace), axis=1)
        net_expect_value = model.get_value(data_feature)
...
state_dict = torch.load("/kaggle_simulations/agent/test_1201_2.pth")
...
def agent(observation, configuration):
    ...
    action = selector.select_action(network_model)
    return action
